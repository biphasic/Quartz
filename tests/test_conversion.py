...
def test_conversion():
    ...
    orig_ann_output = ann(q_values)
    q_ann_output = quartz.quantize_inputs(orig_ann_output, t_max)

    # quartz.from_model(ann, t_max=t_max, batch_size=batch_size)
    snn = quartz.IF(t_max=t_max, rectification=False) 
    ...
    print(orig_ann_output - snn_output)
